/**
 * server.js  —  Selihom Charity Express API (Portfolio Server Deployment)
 *
 * Storage backend: MySQL via db.js connection pool.
 * All API endpoints are identical to the standalone implementation.
 */
...
const parentPublicUploadsDir = path.join(__dirname, "..", "public", "uploads");

[uploadsDir, parentUploadsDir, publicUploadsDir, parentPublicUploadsDir].forEach((dir) => {
...
app.use("/uploads", express.static(parentPublicUploadsDir));
  ...
  try {
    ...
    for (const table of tables) {
      ...
      if (cols.length === 0) {
        ...
        console.log(`✓ Added estimated_delivery_date column to ${table}`);
      }
    }
  } catch (err) {
    console.error("Column migration notice (estimated_delivery_date):", err.message);
  }
}
...
app.post("/api/bookings", async (req, res) => {
  try {
    ...
    const visitorCount= Math.max(1, Math.min(100, parseInt(b.visitorCount, 10) || 1));
  ...
  try {
    ...
    for (const it of (Array.isArray(b.pledgedItems) ? b.pledgedItems : [])) {
      await pool.execute(
        ...
        [id, sanitize(it.itemId, 60), sanitize(it.name, 200), Math.max(0, parseInt(it.quantity, 10) || 0)]);
    }
...
app.post("/api/availability_slots", async (req, res) => {
  try {
    ...
    const maxBookings    = Math.max(1, Math.min(200, parseInt(b.maxBookings, 10) || 10));
    const currentBookings= Math.max(0, parseInt(b.currentBookings, 10) || 0);
...
app.put("/api/availability_slots/:id", async (req, res) => {
  try {
    ...
    `, [sanitize(b.date || b.slot_date, 80), sanitize(b.startTime || b.start_time, 40),
        ...
        Math.max(1, parseInt(b.maxBookings || b.max_bookings, 10) || 10),
        Math.max(0, parseInt(b.currentBookings || b.current_bookings, 10) || 0),
  ...
  try {
    ...
    await pool.execute(`
      ...
      ON DUPLICATE KEY UPDATE
        ...
        urgency=VALUES(urgency),description_en=VALUES(description_en),description_am=VALUES(description_am)
    `, [id, name,
        Math.max(0,parseInt(b.monthlyQty, 10)||0), Math.max(0,parseFloat(b.unitPrice)||0),
        Math.max(0,parseFloat(b.totalMonthly)||0), Math.max(0,parseInt(b.patientsHelped, 10)||0),
  ...
  try {
    ...
    for (const it of (Array.isArray(b.items) ? b.items : [])) {
      await pool.execute(
        ...
        [id, sanitize(it.medicineId, 40), sanitize(it.medicineName, 120), Math.max(1, parseInt(it.quantity, 10) || 1)]);
    }
  ...
  try {
    ...
    for (const it of (Array.isArray(b.items) ? b.items : [])) {
      await pool.execute(
        ...
        [id, sanitize(it.itemId, 40), sanitize(it.itemName, 120), Math.max(1, parseInt(it.quantity, 10) || 1), sanitize(it.custom, 200)]);
    }
...
app.get("/api/site_settings", async (_req, res) => {
  try {
...
app.post("/api/upload", async (req, res) => {
  try {
    ...
    for (const f of fileList) {
      ...
      try {
        const targetPath2 = path.join(parentUploadsDir, uniqueFilename);
...
// ── Backup & Export Database Shape ────────────────────────────────────────

app.get(["/api/backup", "/api/db/export"], async (req, res) => {
  try {
    const [
